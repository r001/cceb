...
async function handleEthWeb3 (current, argv, done, completionFilter) {
  ...
  await Promise.all(completionFilter(async (err, comp) => {
    ...
    if (current.match(/^web3.eth.I/)) {
      ...
      comp = comp.concat(Object.keys(web3.eth.abi).map(key => 'web3.eth.abi.' + key))

    } else if (current.match(/^web3.u/)) {

      comp = comp.concat(Object.keys(web3.utils).map(key => 'web3.utils.' + key))

    } else if (current.match(/^web3.e/)) {

      comp = comp.concat(Object.keys(web3.eth).filter(
        key => ![
          'subscribe',
          'Contract',
          'setProvider',
